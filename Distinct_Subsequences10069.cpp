...
int main()
{
    ...
    {
        string org, want; cin >> org >> want;
        int orglen = org.length(), wantlen = want.length();
        vector<vector<HugeInt>> dp(wantlen + 1, vector<HugeInt>(orglen + 1, 0));
        for (int i = 0; i <= orglen; i++) dp[0][i] = 1;
        for (int i = 1; i <= wantlen; i++)
        {
            char appendc = want[i - 1];
            for (int j = 1; j <= orglen; j++)
            {
                char thisc = org[j - 1];
                if (thisc == appendc) dp[i][j] = dp[i - 1][j - 1] + dp[i][j - 1];
                else dp[i][j] = dp[i][j - 1];
             //   cout << dp[i][j] << " ";
            }
            //cout << endl;
        }
        dp[wantlen][orglen].print(); cout << endl;
    }
}