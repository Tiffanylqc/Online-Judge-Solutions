...
struct SegTree {
    ...
    SegTree(vector<int> A,int N=0) : n(N) {
        A.resize(MAXN);
        this->A=A;
        T.resize(1 << MAXH);
        U.resize(1 << MAXH);
...
int main(){
    ...
    int NumOfCases,NumOfPair,repeat,NumQuery;
    string str;
    ...
    for(int i=1;i<=NumOfCases;i++){
        ...
        while(NumOfPair-->0){
            ...
            for(int i=0;i<repeat;i++){
                for(int j=0;j<str.size();j++){
                    pirates.push_back(str[j]-'0');
                }
            }
        }

        SegTree tree(pirates,pirates.size());
        tree.init();
        cin>>NumQuery;
        char q;
        int a,b;
        ...
        while(NumQuery-->0){
            ...
            if(q=='F'){
                ...
                cout<<"Q"<<QNum<<": "<<tree.query(a,b)<<endl;
                QNum++;
            }
        }
    }
